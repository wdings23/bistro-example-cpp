...
def parse_arguments(file_content):
    argument_start = file_content.find('main0')
    argument_start = file_content.find('(', argument_start) + 1
    ...
    for argument in argument_array:
        ...
        data_type = tokens[variable_type_and_index - 2]
...
##
def output_trace_ray(file_path, shader_argument_info):
...
def main():
    ...
    parsed_output = output_trace_ray(file_base_name + '.metal', shader_argument_info)
